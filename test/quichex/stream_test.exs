defmodule Quichex.StreamTest do
  ...
  describe "stream recv (passive mode)" do
    setup do
      config =
        Config.new!()
        |> Config.set_application_protos(["h3"])
    ...
    test "stream_recv/3 with default max_len", %{config: config} do
      {:ok, conn} =
        Connection.connect(
          host: "cloudflare-quic.com",
          port: 443,
      ...
      # Try to receive (will likely error with "done" since no data)
    ...
    test "stream_recv/3 with custom max_len", %{config: config} do
      {:ok, conn} =
        Connection.connect(
          host: "cloudflare-quic.com",
          port: 443,
      ...
      # Try to receive with custom max_len
